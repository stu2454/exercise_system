...
export function ClientDemoOnboarding(props: ClientDemoOnboardingProps) {
  ...
  if (stage === "prepare-space") return <div className="client-demo-onboarding"><SetupProgress stage={stage} /><main className="client-demo-card"><p className="eyebrow">Before we begin</p><h1>Prepare Your Space</h1><ul><li>Place your laptop or device on a stable surface.</li><li>Position the camera so it faces you directly.</li><li>Step back so most or all of your body is visible.</li><li>Make sure your arms and legs can move within the camera view.</li><li>Use a well-lit area and avoid strong light behind you.</li><li>Make sure you have enough clear space to move safely.</li></ul><p className="client-demo-safety"><strong>Stop if you feel unwell or uncomfortable.</strong></p><button className="participant-primary-action" onClick={onNext}>CONTINUE TO CAMERA</button></main></div>;
  ...
  if (stage === "camera-setup") {
    ...
    return <div className="client-demo-onboarding"><SetupProgress stage={stage} /><main className="client-demo-camera-layout"><div><p className="eyebrow">Camera</p><h1>{failed ? "Camera Access Needed" : "Camera Setup"}</h1><p>{failed ? "Camera access is required for this demonstration. Please allow access in your browser settings and try again." : "We use your camera so the demonstration can display and track your movement."}</p><p className="client-demo-privacy">Camera images and movement processing remain on your device.</p></div><CameraSurface cameraState={cameraState} videoRef={videoRef} canvasRef={canvasRef} message={message} /><button className="participant-primary-action" onClick={onEnableCamera} disabled={cameraState.status === "requesting"}>{cameraState.status === "requesting" ? "STARTING CAMERA…" : failed ? "TRY AGAIN" : "ENABLE CAMERA"}</button></main></div>;
  }
